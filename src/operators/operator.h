...
class Operator {
 public:
    ...
    Operator(std::string opName, std::unique_ptr<RunTimeString> param, bool negation)
        : m_match_message(""),
        m_negation(negation),
        m_op(opName),
        m_param(""),
    ...
    virtual std::string resolveMatchMessage(Transaction *t, std::string key, std::string value);
